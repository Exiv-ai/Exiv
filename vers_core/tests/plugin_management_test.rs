use sqlx::SqlitePool;
use std::sync::Arc;
use vers_shared::{PluginConfig, PluginFactory, ServiceType, Plugin, PluginManifest};
use vers_core::managers::PluginManager;
...
impl vers_shared::PluginCast for MockPlugin {
...
async fn test_plugin_bootstrap_with_valid_seal() {
    ...
    let mut manager = PluginManager::new(pool.clone(), vec![], 5, 10);
...
impl vers_shared::PluginCast for InvalidSealPlugin {
    fn as_any(&self) -> &dyn std::any::Any { self }
}
