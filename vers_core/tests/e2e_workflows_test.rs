use sqlx::SqlitePool;
use std::sync::Arc;
use vers_core::AppState;
use vers_core::config::AppConfig;
use vers_core::managers::{PluginRegistry, AgentManager, PluginManager, SystemMetrics};
use vers_core::DynamicRouter;
use vers_shared::{VersEvent, VersEventData, VersMessage, MessageSource};
...
async fn create_test_app_state() -> Arc<AppState> {
    ...
    vers_core::db::init_db(&pool, "sqlite::memory:").await.unwrap();
    ...
    let rate_limiter = Arc::new(vers_core::middleware::RateLimiter::new(10, 20));
...
async fn test_user_message_to_response_flow() {
    ...
    state.agent_manager.create_agent(
        ...
        vec![vers_shared::CapabilityType::Reasoning],
    ).await.unwrap();
    ...
    let event = Arc::new(VersEvent::new(
        VersEventData::MessageReceived(user_message.clone())
    ));
    ...
    match &received.data {
        VersEventData::MessageReceived(msg) => {
...
async fn test_permission_grant_flow() {
    ...
    vers_core::update_permission_request(&state.pool, "req-123", "approved", "admin")
        .await
        .unwrap();

    // Send PermissionGranted event
    let event = Arc::new(VersEvent::new(
        VersEventData::PermissionGranted {
    ...
    match &received.data {
        VersEventData::PermissionGranted { plugin_id, permission } => {
            assert_eq!(plugin_id, "test.plugin");
            assert_eq!(permission, &vers_shared::Permission::NetworkAccess);
        }
...
async fn test_config_update_flow() {
    ...
    match &received.data {
        VersEventData::ConfigUpdated { plugin_id, config } => {
            assert_eq!(plugin_id, "test.plugin");
...
async fn test_agent_creation_with_memory_context() {
    ...
    assert!(agent.required_capabilities.contains(&vers_shared::CapabilityType::Memory));
