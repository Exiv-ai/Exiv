...
use vers_core::managers::{PluginRegistry, PluginManager, SystemMetrics};
use vers_core::DynamicRouter;
use vers_shared::{VersEvent, VersEventData};

/// Helper to create EventProcessor for testing
async fn create_test_processor(max_history_size: usize) -> (Arc<EventProcessor>, Arc<RwLock<VecDeque<Arc<VersEvent>>>>) {
    let pool = SqlitePool::connect("sqlite::memory:").await.unwrap();

    let registry = Arc::new(PluginRegistry::new(5, 10));
    let plugin_manager = Arc::new(PluginManager::new(pool.clone(), vec![], 30, 10));
...
async fn test_event_history_size_limit() {
    ...
    // Manually add 1500 events to the history
    {
        ...
        for i in 0..1500 {
            let event = Arc::new(VersEvent::new(
                VersEventData::SystemNotification(format!("Event {}", i))
            ));
    ...
    if let Some(oldest) = hist.front() {
        if let VersEventData::SystemNotification(msg) = &oldest.data {
...
async fn test_time_based_cleanup() {
    ...
    // Add events with old timestamps (manually)
    {
        ...
        for i in 0..10 {
            let mut event = VersEvent::new(
                VersEventData::SystemNotification(format!("Old Event {}", i))
            );
        ...
        for i in 0..10 {
            let mut event = VersEvent::new(
                VersEventData::SystemNotification(format!("Recent Event {}", i))
            );
    ...
    for event in hist.iter() {
        if let VersEventData::SystemNotification(msg) = &event.data {
...
async fn test_cleanup_task_integration() {
    ...
    // Add some old events
    {
        ...
        for i in 0..5 {
            let mut event = VersEvent::new(
                VersEventData::SystemNotification(format!("Old {}", i))
            );
