...
fn binary_name() -> &'static str {
    if cfg!(windows) { "vers_system.exe" } else { "vers_system" }
}
...
fn env_template(prefix: &Path, api_key: &str) -> String {
    ...
    let db_path = prefix.join("data").join("vers_memories.db");

    format!(
        r#"# ============================================================
# VERS System Configuration
# Generated by vers_system install on {timestamp}
# ============================================================
...
/// Install VERS to the specified prefix directory
pub async fn install(
    ...
    if !env_path.exists() {
        ...
        info!("🔑 Generated .env with VERS_API_KEY");
        println!("  VERS_API_KEY has been auto-generated. Save it securely:");
        println!("  {}", api_key);
    } else {
    ...
    if service {
        if cfg!(windows) {
            println!("  As service:       sc.exe start VERS");
        } else {
...
/// Uninstall VERS from the specified prefix directory
pub async fn uninstall(prefix: PathBuf) -> anyhow::Result<()> {
    println!("=== VERS System Uninstaller ===");
