...
use vers_shared::{
    ...
    VersId, Permission, PluginRuntimeContext
};
...
async fn redispatch_plugin_event(
    ...
    plugin_id: String,
    trace_id: VersId,
    new_event_data: vers_shared::VersEventData,
    current_depth: u8,
...
impl PluginManager {
    pub fn new(pool: SqlitePool, allowed_hosts: Vec<String>, event_timeout_secs: u64, max_event_depth: u8) -> Self {
        Self {
            pool: pool.clone(),
    ...
    pub fn register_builtins(&mut self) {
        ...
        if discovered_count == 0 {
            ...
            error!("   1. Plugin crates are added to vers_core/Cargo.toml");
            error!("   2. Plugin crates are imported in vers_core/src/lib.rs");
    ...
    async fn bootstrap_plugin(
        ...
        let (p_tx, mut p_rx) = tokio::sync::mpsc::channel::<vers_shared::VersEventData>(100);
        ...
        let network_capability = if permissions.contains(&Permission::NetworkAccess) {
            self.get_capability_for_permission(&Permission::NetworkAccess)
                .and_then(|c| {
                    let vers_shared::PluginCapability::Network(net) = c;
                    Some(net)
                })
        } else {
            None
...
struct AgentRow {
    ...
    required_capabilities: sqlx::types::Json<Vec<vers_shared::CapabilityType>>,
...
impl AgentManager {
    ...
    pub async fn get_agent_config(&self, agent_id: &str) -> anyhow::Result<(AgentMetadata, String)> {
        let row: AgentRow = sqlx::query_as(
            "SELECT id, name, description, status, default_engine_id, required_capabilities, metadata FROM agents WHERE id = ?"
        )
        .bind(agent_id)
        .fetch_one(&self.pool)
        .await?;
        ...
        let metadata = AgentMetadata {
            ...
            status: row.status,
            default_engine_id: Some(row.default_engine_id.clone()),
    ...
    pub async fn list_agents(&self) -> anyhow::Result<Vec<AgentMetadata>> {
         let rows: Vec<AgentRow> = sqlx::query_as(
            "SELECT id, name, description, status, default_engine_id, required_capabilities, metadata FROM agents"
        )
        .fetch_all(&self.pool)
        .await?;
        
        let agents: Vec<AgentMetadata> = rows.into_iter().map(|row| {
            AgentMetadata {
                id: row.id,
                name: row.name,
                description: row.description,
    ...
    pub async fn create_agent(
        ...
        required_capabilities: Vec<vers_shared::CapabilityType>,
        ...
        sqlx::query("INSERT INTO agents (id, name, description, default_engine_id, status, metadata, required_capabilities) VALUES (?, ?, ?, ?, 'offline', ?, ?)")
            .bind(&id_str)
            .bind(name)
            .bind(description)
            .bind(default_engine)
