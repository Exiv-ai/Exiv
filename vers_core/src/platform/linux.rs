...
const SERVICE_FILE: &str = "/etc/systemd/system/vers.service";

/// Generate systemd service unit file content
fn service_unit(prefix: &Path, user: &str) -> String {
    let exec_start = prefix.join("vers_system");
    format!(
        r#"[Unit]
...
/// Register VERS as a systemd service
