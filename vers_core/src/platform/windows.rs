...
const SERVICE_NAME: &str = "VERS";

/// Register VERS as a Windows Service via sc.exe
pub fn install_service(prefix: &Path, _user: Option<&str>) -> anyhow::Result<()> {
    let exe_path = prefix.join("vers_system.exe");
...
pub fn execute_swap(target: std::path::PathBuf, pid: u32) -> anyhow::Result<()> {
    eprintln!("VERS swap-exe: waiting for PID {} to exit...", pid);
    ...
    eprintln!("VERS swap-exe: parent exited, performing binary swap...");
    ...
    eprintln!("VERS swap-exe: binary updated. Restarting service...");
