...
/// Returns current VERS version and build target (public, no auth).
...
pub async fn check_handler(
    ...
    let response = client
        .get(&url)
        .header("User-Agent", format!("VERS-SYSTEM/{}", current_version))
    ...
    let expected_asset_name = format!("vers_system-{}", target);
...
pub async fn apply_handler(
    ...
    let audit_entry = crate::db::AuditLogEntry {
        ...
        target_id: Some("vers_system".to_string()),
        permission: None,
    ...
    let envelope = crate::EnvelopedEvent::system(
        vers_shared::VersEventData::SystemNotification(format!(
