...
// Reusable infrastructure for VERS performance benchmarks
// Pattern inspired by: vers_core/tests/handlers_http_test.rs:18-60
...
pub async fn create_bench_app_state() -> Arc<AppState> {
    ...
    let rate_limiter = Arc::new(vers_core::middleware::RateLimiter::new(100, 200));
...
pub fn create_test_event(message: String) -> Arc<VersEvent> {
    let msg = VersMessage::new(
        MessageSource::User {
    ...
    Arc::new(VersEvent::new(VersEventData::MessageReceived(msg)))
}

/// Create an enveloped event for dispatch benchmarks
pub fn create_enveloped_event(message: String) -> vers_core::EnvelopedEvent {
    vers_core::EnvelopedEvent {
