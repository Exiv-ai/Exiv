...
Procedural macros for defining VERS plugins with automatic manifest generation, icon embedding, and capability registration.

## Usage

```rust
use vers_macros::vers_plugin;
use vers_shared::{Plugin, PluginConfig, PluginFactory};
...
```bash
VERS_SKIP_ICON_EMBED=0 cargo build --release
```
...
Same as VERS-SYSTEM project license.
