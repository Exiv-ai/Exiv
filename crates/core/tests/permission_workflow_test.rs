...
async fn test_update_effective_permissions_adds_permission() {
    ...
    registry.update_effective_permissions(plugin_id, Permission::NetworkAccess).await;
...
async fn test_update_effective_permissions_multiple_types() {
    ...
    registry.update_effective_permissions(plugin_id, Permission::NetworkAccess).await;
    registry.update_effective_permissions(plugin_id, Permission::InputControl).await;
...
async fn test_permissions_are_isolated_between_plugins() {
    ...
    registry.update_effective_permissions(plugin_a, Permission::NetworkAccess).await;
...
async fn test_plugin_registry_list_plugins_empty() {
    ...
    assert!(manifests.is_empty(), "Empty registry must return empty manifest list");
}
...
async fn test_plugin_registry_get_engine_missing_returns_none() {
    ...
    assert!(result.is_none(), "Getting nonexistent plugin must return None");
}
...
async fn test_plugin_registry_find_memory_empty_returns_none() {
    ...
    assert!(result.is_none(), "find_memory on empty registry must return None");
}
...
async fn test_db_permission_grant_roundtrip() {
    ...
    let manager = exiv_core::managers::PluginManager::new(
        pool.clone(),
        vec![],
        5,
        10,
    ...
    manager.grant_permission("test.plugin", Arc::new(Permission::NetworkAccess).as_ref().clone()).await.unwrap();
    ...
    assert!(perms.contains(&Permission::NetworkAccess), "Granted permission must be persisted");
}
