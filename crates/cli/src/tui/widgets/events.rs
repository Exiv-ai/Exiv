...
pub fn render(f: &mut Frame, area: Rect, app: &App, is_active: bool) {
    ...
    let items: Vec<ListItem> = app.events.iter().rev().map(|event| {
        ...
        let (type_color, detail) = match event_type {
            ...
            "AgentPowerChanged" => {
                let agent = data.get("agent_id")
                    .and_then(|a| a.as_str())
                    .unwrap_or("?");
                let on = data.get("enabled").and_then(|e| e.as_bool()).unwrap_or(false);
                (Color::Magenta, format!("{agent} {}", if on { "ON" } else { "OFF" }))
            }
            "SystemNotification" => {
                (Color::Yellow, "System notification".to_string())
            }
            "ConfigUpdated" => {
                let plugin = data.get("plugin_id").and_then(|p| p.as_str()).unwrap_or("?");
                (Color::Yellow, format!("{plugin} config"))
            }
            _ => {
                (Color::DarkGray, event_type.to_string())
            }
        };
