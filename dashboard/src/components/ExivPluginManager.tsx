...
import { Puzzle, Shield, CheckCircle2, AlertTriangle, Save, Filter, Settings, ExternalLink, Terminal, ChevronDown, ChevronRight, Hash, Database, MousePointer2 } from 'lucide-react';
...
export function ExivPluginManager() {
  ...
  const { plugins, isLoading, refetch: refetchPlugins } = usePlugins();
  ...
  // Sync editingPlugins when plugins update from hook
  useEffect(() => {
    if (plugins.length > 0) {
      setEditingPlugins(JSON.parse(JSON.stringify(plugins)));
    }
  }, [plugins]);
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex flex-wrap gap-1.5 mt-auto">
                            ...
                            {!isVerified && (
                              <span className="px-2 py-0.5 bg-amber-500/20 text-amber-500 rounded text-[9px] font-black uppercase tracking-tighter">
                                UNVERIFIED
                              </span>
                            )}
                          </div>
