...
impl PythonBridgePlugin {
    pub async fn new_plugin(config: PluginConfig) -> anyhow::Result<Self> {
        ...
        let base_canonical = base_dir.canonicalize()
            .map_err(|e| {
                match e.kind() {
                    std::io::ErrorKind::NotFound => anyhow::anyhow!("Scripts directory not found: {}", base_dir.display()),
                    std::io::ErrorKind::PermissionDenied => anyhow::anyhow!("Permission denied accessing scripts directory: {}", base_dir.display()),
                    _ => anyhow::anyhow!("Failed to resolve scripts directory: {} ({})", base_dir.display(), e),
                }
            })?;
        let candidate_canonical = candidate_path.canonicalize()
            .map_err(|e| {
                ...
                match e.kind() {
                    std::io::ErrorKind::NotFound => anyhow::anyhow!("Script file not found: {}", script_path),
                    std::io::ErrorKind::PermissionDenied => anyhow::anyhow!("Permission denied accessing script: {}", script_path),
